...
Each of the 3 workers is bound to one **viewport** (Desktop 1920×1080 / Tablet 1024×1366 / Mobile 500×800 -
see `references/coverage-areas.md` for why Mobile is fixed at 500×800 rather than a true phone width)
**and** one **persistent account** for its entire lifetime - it logs in once and never switches roles, which
also eliminates the login-collision gotcha (`mcp-crawl-procedure.md`'s Per-role login point 7) the same way
the original persona-split design did, just combined with the viewport split rather than instead of it. This
maps directly onto the criteria sheet's own shape (Desktop/Tablet/Mobile columns are exactly the 3 workers'
outputs) while *also* getting simultaneous multi-tier and multi-account coverage for free:

- **Worker 1 = Desktop (1920×1080) + the real admin account.** Full crawl of every screen at desktop
  viewport, using the actual admin login. **Also owns all write-path testing** (Add/Edit/Delete/Export,
  admin-triggered email flows, etc.) and is responsible for creating the other two workers' disposable
  accounts before the fan-out - see "Phase 0" below.
- **Worker 2 = Tablet (1024×1366, iPad Pro 12.9") + disposable account #1**, created by Worker 1 in Phase 0 as an
  `+alias` of the tester's own inbox (e.g. `you+qatablet@yourcompany.com`) and assigned to one tier the
...
- **Worker 3 = Mobile (500×800) + disposable account #2**, a second `+alias`
  (e.g. `you+qamobile@yourcompany.com`) assigned to a *different* tier than Worker 2's. Same shape as
...
Desktop + admin · Worker 2: Tablet + `tier-a`-tier disposable account · Worker 3: Mobile +
`tier-b`-tier disposable account."
...
## Prerequisite
Confirm the 3 named Selenium MCP servers (`selenium1`/`selenium2`/`selenium3`, each its own
`@angiejones/mcp-selenium` process) exist and are connected - see `mcp-crawl-procedure.md`'s "Parallel setup"
section for the exact `claude mcp list` check and `claude mcp add` commands if missing. Do not proceed into
the phases below until all 3 show `✔ Connected`.
...
`you+qatablet@yourcompany.com` / `you+qamobile@yourcompany.com` - never a real third-party address,
so the Gmail MCP can verify any email-based flow across all of them from that single inbox). This is the
**only** account-setup path - do not create accounts via the admin Add-User flow instead; asking the admin
panel to create these every run was the single biggest token cost in Phase 0 (the Users-tab round trip plus
its large Users-table page-source dumps), and is now unnecessary since the credentials already exist.

If the user hasn't supplied these credentials yet when a parallel run is requested, stop and ask for them
(3 sets: admin + 2 member accounts on different tiers, per the platform spec's priority - untested tiers, or
tiers with real business weight) rather than falling back to admin-created accounts.

Before the parallel fan-out, do a quick sanity login check per account (not a full crawl) - a bad credential
...
prompt must:
1. State its assigned viewport (Desktop 1920×1080 / Tablet 1024×1366 / Mobile 500×800) and its assigned
   Selenium MCP server (`mcp__selenium1__*` and so on) - **explicitly forbid it from calling any other
   server's tools**. Because this server has no mid-session resize/`emulate()` call, the viewport must be
   baked into its **first** `start_browser` call as a launch option via `options.arguments` (a plain string
   array of Chrome CLI flags - confirmed shape; there is no `mobileEmulation` field at all):
   `--window-size=1920,1080` for Desktop, `--window-size=1024,1366 --user-agent=<iPad UA>` for Tablet,
   `--window-size=500,943 --user-agent=<iPhone UA>` for Mobile (confirmed to produce exactly 500×800 - the
   fixed Mobile target for this skill, per `references/coverage-areas.md`) - see `mcp-crawl-procedure.md`'s
   "Per-screen capture" section for the full recipe. There's nothing to "re-confirm after every navigation"
   the way chrome-devtools' `emulate()` needed - the viewport is fixed
   for the whole session once `start_browser` launches, which removes that particular false-dead-click cause
   entirely (there's no stray-viewport-left-over-from-a-prior-check failure mode when the viewport can't
   change mid-session in the first place).
2. Carry the full context a classic-mode agent would need: the platform base URL, the in-scope screen list
   ...
   to discover it, per `mcp-crawl-procedure.md`'s Endpoint discovery), the *full* `references/coverage-areas.md`
...
- Instruct **Workers 2 and 3** to, later in their own crawl (not immediately - leave a real gap so Worker 1's
  ...
  its content matches what Worker 1 sent. `wait_for` is text-appearance only (no time-based wait) - either
  wait on text the mention itself would produce (a badge count, a notification string), or space this check
  naturally later in the worker's own crawl sequence rather than trying to engineer an explicit delay.
